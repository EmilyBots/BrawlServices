...
// ─── Top-level imports (not inside request handlers) ────────────────────────
...
// NOTE: This endpoint is unauthenticated — consider adding an API key check
// if this app is publicly accessible.
...
vouches: {
total: vouches.rows[0].total,
avgRating: parseFloat(vouches.rows[0].avg || 0).toFixed(2),
},
});
} catch (e) {
...
await db.query(
  ...
  [
    paymentIntentId,
    JSON.stringify({ sessionId, paymentIntentId, amount: session.amount_total / 100 }),
    order.id,
    method,
  ]
);
...
try {
  const { EmbedBuilder } = require('discord.js');
  const guild = client.guilds.cache.get(process.env.GUILD_ID);
  const methodLabel = method === 'applepay' ? '🍎 Apple Pay'
...
// ─── Start server, then login to Discord ────────────────────────────────────
