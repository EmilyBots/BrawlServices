...
// ─── Start Express FIRST so Railway health check passes immediately ──────────
const app  = express();
...
app.get('/',       (req, res) => res.json({ status: 'online', uptime: Math.floor(process.uptime()), timestamp: new Date().toISOString() }));
...
app.listen(PORT, () => {
  ...
  // ─── Discord client (started after Express is listening) ──────────────────
  ...
  // Make client accessible to payment routes defined above
  ...
  // ─── Run DB migrations then login ──────────────────────────────────────────
  if (!process.env.DISCORD_TOKEN) {
    console.error('⚠️  DISCORD_TOKEN not set — bot will not connect to Discord');
    return;
  }
