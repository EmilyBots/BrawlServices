...
module.exports = {
  ...
  async execute(member) {
    ...
    // Log member leave
    logger.memberLeave(member.client, guild.id, member);
    ...
    try {
      ...
      try {
        ...
        if (log) {
          ...
          inviteInfo =
            `\n📨 Invited by ${inviterTag} via \`${code}\` — ` +
            `they now have **${inviterTotal}** invite${inviterTotal !== 1 ? 's' : ''}`;
          ...
          try {
            ...
            if (invite && invite.maxUses === 1) {
              await invite.delete(
                `Auto-removed: single-use invite, invitee ${member.user.username} left`
              );
