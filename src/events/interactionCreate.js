...
if (interaction.isButton()) {
  ...
  if (id.startsWith('cbk_confirm_')) {
    try {
      ...
      console.log(`[cbk_confirm] sType=${sType} dStr=${dStr} time=${time}`);
      ...
      if (!pricing) {
        console.error(`[cbk_confirm] No pricing found for sType: ${sType}`);
        return interaction.editReply({
          embeds: [error('Error', `Unknown session type: ${sType}`)],
          components: [], files: [],
        });
      }
      ...
      console.log(`[cbk_confirm] Inserting session ${sessionId}`);
      ...
      console.log(`[cbk_confirm] Creating ticket`);
      ...
      console.log(`[cbk_confirm] Sending messages to ticket ${ticketChannel.id}`);
      ...
      console.log(`[cbk_confirm] Messages sent successfully`);
      ...
      console.error('❌ cbk_confirm_ error:', confirmErr);
      return interaction.editReply({
        embeds: [error('Something went wrong', confirmErr.message || 'Unknown error')],
        components: [], files: [],
      });
    }
  }
