...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isButton()) {
      ...
      // vouch_submit needs showModal — must NOT deferUpdate first
      if (id === 'vouch_submit') {
      ...
      try {
        ...
        if (id === 'panel_vouches') {
          return interaction.editReply(panels.vouchPanel());
        }
        ...
        if (id.startsWith('claim_order_')) {
          ...
          try {
            ...
            const disabledRow = new ARB().addComponents(
              new BB().setCustomId(`claimed_${interaction.user.id}`).setLabel(`Claimed by ${interaction.user.username}`).setStyle(BS.Success).setDisabled(true)
            );
          ...
          if (method === 'applepay' || method === 'googlepay') {
            ...
            try {
              ...
              const payRow = new ARB().addComponents(
                new BB().setLabel(`Pay with ${method === 'applepay' ? 'Apple Pay' : 'Google Pay'}`).setEmoji(method === 'applepay' ? '🍎' : '🔵').setStyle(BS.Link).setURL(checkoutUrl)
              );
              const payEmbed = base(COLORS.PRIMARY).setTitle(`${label} Checkout`)
                .setDescription(`**Order:** \`#${shortId}\`\n**Amount:** **€${parseFloat(o.price).toFixed(2)}**\n\nClick the button below to pay securely.\n> 🔒 *Powered by Stripe*`);
              if (process.env.PAYMENT_LOG_CHANNEL_ID) {
                const logCh = interaction.guild.channels.cache.get(process.env.PAYMENT_LOG_CHANNEL_ID);
                if (logCh) logCh.send({ embeds: [base(COLORS.WARNING).setTitle(`💳 ${label} Checkout Started`).setDescription(`<@${interaction.user.id}> — \`#${shortId}\` — **€${o.price}**`)] });
              }
          ...
          if (method === 'paypal') {
            ...
            try {
              ...
              const { paypalOrderId, approveUrl } = await paypal.createOrder({
                orderId, amount: o.price,
                description: `Brawl Services™ – ${o.service_type} (${o.from_rank} → ${o.to_rank})`,
                returnUrl: `${webUrl}/payment/success`,
                cancelUrl: `${webUrl}/payment/cancel?order=${shortId}`,
              });
              ...
              const { ActionRowBuilder: ARB, ButtonBuilder: BB, ButtonStyle: BS } = require('discord.js');
              const payRow = new ARB().addComponents(
                new BB().setLabel('Pay with PayPal').setEmoji('💲').setStyle(BS.Link).setURL(approveUrl)
              );
              const payEmbed = base(COLORS.PRIMARY).setTitle(`💲 PayPal Checkout`)
                .setDescription(`**Order:** \`#${shortId}\`\n**Amount:** **€${parseFloat(o.price).toFixed(2)}**\n\nClick below to pay via PayPal.\n> 🔒 *Powered by PayPal*`);
              if (process.env.PAYMENT_LOG_CHANNEL_ID) {
                const logCh = interaction.guild.channels.cache.get(process.env.PAYMENT_LOG_CHANNEL_ID);
                if (logCh) logCh.send({ embeds: [base(COLORS.WARNING).setTitle(`💳 PayPal Checkout Started`).setDescription(`<@${interaction.user.id}> — \`#${shortId}\` — **€${o.price}**`)] });
              }
              return interaction.followUp({ embeds: [payEmbed], components: [payRow], ephemeral: true });
            } catch (err) {
        ...
        if (id.startsWith('cbk_day_')) {
          ...
          const { rows: booked } = await db.query(
            `SELECT TO_CHAR(scheduled_at AT TIME ZONE 'CET', 'HH24:MI') as time FROM coaching_sessions WHERE DATE(scheduled_at AT TIME ZONE 'CET') = $1 AND status NOT IN ('cancelled')`,
            [dStr]
          );
        ...
        if (id.startsWith('cbk_prevmonth_') || id.startsWith('cbk_nextmonth_')) {
          ...
          const { rows: booked } = await db.query(
            ...
            [new Date(y, m, 1).toISOString(), new Date(y, m + 1, 0).toISOString()]
          );
        ...
        if (id.startsWith('cbk_confirm_')) {
          ...
          const { rows: conflict } = await db.query(
            `SELECT id FROM coaching_sessions WHERE DATE(scheduled_at AT TIME ZONE 'CET') = $1 AND TO_CHAR(scheduled_at AT TIME ZONE 'CET', 'HH24:MI') = $2 AND status NOT IN ('cancelled')`,
            [dStr, time]
          );
          if (conflict.length) {
            return interaction.editReply({ embeds: [error('Slot Taken', `**${time} CET on ${dStr}** was just booked! Please go back and choose another slot.`)], components: [], files: [] });
          }
          ...
          await db.query(
            `INSERT INTO coaching_sessions (id, user_id, guild_id, session_type, duration_hours, price, scheduled_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [sessionId, interaction.user.id, interaction.guild.id, sType, parseInt(pricing.label.match(/\d+/)?.[0] || 1), pricing.price, scheduledAt.toISOString()]
          );
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      try {
        if (id === 'order_service_select') {
          ...
          return interaction.editReply({ embeds: [base(COLORS.INFO).setTitle(`📦 Place Order`).setDescription(`Use \`/order create\` with service \`${service}\` to complete your order!`)], components: [], files: [] });
        }
